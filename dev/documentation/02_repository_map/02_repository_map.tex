...
This file converts user config dictionaries into normalized branch specifications. In practice, many parts of the runner depend on the \texttt{BranchSpec} dataclass rather than directly reading raw config dictionaries.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \texttt{runtime.py}: solver execution and result-category conversion,
    \item \texttt{aggregation.py}: condition-level summaries,
    \item \texttt{plotting.py}: metric graphs,
    \item \texttt{io\_utils.py}: output folder management and JSON/CSV writing,
...
\begin{itemize}[leftmargin=2em]
    \item static CBS and low-level A*,
    \item time-expanded CBS and low-level A*,
