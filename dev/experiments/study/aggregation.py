...
def aggregate_mapping(records: list[MappingRunRecord]) -> tuple[float | None, float | None, float | None]:
    ...
    path_values = [
        record.average_path_length
        for record in records
        if record.solved_run and record.average_path_length is not None
    ]
...
def build_condition_aggregate(
    *,
    ...
    cyclic_replayed_all_paired_configs = cyclic_counted_runs >= paired_run_configurations

    if classical_reached_counted_quota and cyclic_replayed_all_paired_configs:
        notes = "jointly_viable_paired_sampling"
    elif not classical_reached_counted_quota:
        notes = "counted_quota_not_reached_before_safeguard"
    else:
        notes = "cyclic_retained_pairs_incomplete"
    ...
    return ConditionAggregate(
        ...
        num_classical_unsolvable_runs=0,
        num_classical_setup_failed_runs=0,
        ...
        num_cyclic_unsolvable_runs=0,
        num_cyclic_setup_failed_runs=0,
