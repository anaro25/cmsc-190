...
The most important recent change is that the dynamic pipeline no longer supports a many-to-one shared campus goal. Instead, the assignment layer now exposes:
\begin{itemize}[leftmargin=2em]
    ...
    \item one-to-one matching with optional individual reachability.
\end{itemize}

That means the dynamic pipeline still shares \emph{maps}, but each run configuration can now differ in a richer and more explicit way than the old \texttt{single\_cell\_target} toggle allowed.
