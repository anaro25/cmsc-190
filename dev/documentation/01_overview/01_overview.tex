...
The project is therefore not mainly about inventing a new high-level MAPF solver. Instead, it studies what happens when the \emph{same underlying planning problem family} is presented through two different movement topologies.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item dispersed vs.\ clustered target placement,
...
At present, the code is best understood as a generalized branch runner with specialized preparation logic:
\begin{enumerate}[leftmargin=2em]
    ...
    \item solve each retained configuration under both mappings,
    \item aggregate the retained paired results,
    \item write structured records, graphs, and selected Pillow visualizations.
\end{enumerate}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item dispersed sets avoid internal 8-neighbor adjacency, while clustered sets are sampled as directly adjacent 8-neighbor-connected components.
\end{itemize}
