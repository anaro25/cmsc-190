...
The new \texttt{enhanced\_CBS} switch is global. Do not quietly make solver choice branch-specific unless that experiment-design change is intentional and documented. The current design uses one solver family for the whole run so solver behavior remains comparable across branches. However, each branch now stores its own editable \texttt{ECBS\_suboptimality} value in \texttt{master\_config.py}; if ECBS behavior changes, keep the propagation of that branch-level value into branch specs, runtime dispatch, solver internals, and recorded metadata consistent.
