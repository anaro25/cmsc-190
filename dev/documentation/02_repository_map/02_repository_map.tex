...
\begin{itemize}[leftmargin=2em]
    ...
    \item \texttt{raw\_data\_store.py}: branch-local persisted raw MAPF payload storage under \texttt{dev/raw\_mapf\_data/},
...
\]
with subfolders for metadata, records, aggregates, graphs, logs, and visualizations. Persisted branch-local raw MAPF payloads are stored separately under:
\[
\texttt{dev/raw\_mapf\_data/<map\_type>/}
\]
so they survive later graph or visualization regeneration runs. This split between persisted raw data and disposable presentation outputs is now part of the practical contract of the project.
