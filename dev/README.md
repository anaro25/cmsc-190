...
## Additional ECBS weight-sweep experiment

A separate additional experiment is available for comparing `ECBS + classical mapping`
against `ECBS + cyclic mapping` across ECBS suboptimality factors. This workflow keeps
the agent count fixed per map and uses the ECBS weight as the x-axis.

To run it, edit `dev/main.py` and select:
...
Then edit `dev/master_config_additional_experiment.py`. The additional experiment uses
its own config file so the main experiment in `dev/master_config.py` remains untouched.
Outputs are written under `dev/output_additional_exp/`.

The included additional-experiment maps are:

- `reference_rahman_32x32`, loaded from `dev/inputs/reference_map/reference_map.png`
- `static_artificial_weight_sweep`
- `static_port_weight_sweep`

The additional graphs are line graphs, not barbell charts. They keep the same basic
classical/cyclic color design as the main graph style. Computation time halted and
conflicts are plotted; average path length is recorded but not plotted.
