...
The comparison is organized across thirty-two exact map configurations, grouped under eight map categories:
\begin{enumerate}[leftmargin=2em]
    \item \texttt{static\_artificial},
    \item \texttt{dynamic\_artificial},
    \item \texttt{static\_port},
    \item \texttt{dynamic\_port},
    \item \texttt{static\_campus\_area\_1},
    \item \texttt{dynamic\_campus\_area\_1},
    \item \texttt{static\_campus\_area\_2},
    \item \texttt{dynamic\_campus\_area\_2}.
\end{enumerate}

The map types differ in how maps are generated or loaded, how starts and goals are positioned, and whether the environment is static or time-varying. The common comparison axis is always:
\[
...
The exact map configurations are not cosmetic labels. They differ in meaningful ways:
\begin{itemize}[leftmargin=2em]
    \item static vs.\ dynamic environments,
    \item artificial generation vs.\ image-based input,
    \item traditional dispersed/clustered agent and target placement,
    \item campus dispersed/single-cell agent and target placement,
...
\subsection{Assignment Contract}

Traditional branches use one-to-one MAPF assignments for dispersed or clustered targets. Campus crowd-simulation branches may use \texttt{single} for agents, targets, or both; single-cell targets are many-to-one shared destinations where agents disappear after arrival.
...
\begin{enumerate}[leftmargin=2em]
    \item resolve exact branch specifications from \texttt{SELECTED\_MAP\_CONFIGS} in \texttt{master\_config.py},
    \item prepare either a fresh static run context or a shared dynamic branch state,
    \item sample and solve the capacity-search and comparative configurations under both mappings,
    \item write the author-facing evaluation text under \texttt{metrics\_data\_inspection},
    \item automatically write an independent exact-configuration Results-ready package under \texttt{metrics\_data},
    \item save the designated successful trajectories under \texttt{frame\_by\_frame},
    \item optionally regenerate Pillow visualizations from those frame-by-frame packages.
\end{enumerate}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item one unique goal per agent for dispersed and clustered target modes, or one shared target cell for campus single-target mode,
    \item dispersed sets prioritize internal 8-neighbor clearance. The per-map \texttt{strict\_dispersed\_8\_neighbor\_clearance} toggle makes this mandatory when \texttt{True}; when \texttt{False}, adjacent unique cells are used only after separated placement can no longer fill the requested set. Clustered sets are controlled by the global \texttt{compact\_clustering} switch: compact directly adjacent clusters when \texttt{True}, and connected one-cell-gap clusters when \texttt{False}.
\end{itemize}
