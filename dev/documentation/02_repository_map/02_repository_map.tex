...
\[
\texttt{dev/raw\_mapf\_data/<map\_type>/}
\]
so they survive later graph or visualization regeneration runs. The persisted raw-data tree is now organized as a branch root containing a manifest, metadata files, and one subfolder per reported condition, which keeps large experiments more navigable than a single all-in-one pickle. This split between persisted raw data and disposable presentation outputs is now part of the practical contract of the project.
