...
Downstream reading, analysis, or prompting may depend on the current output tree and JSON/CSV file names. Renaming those outputs should be treated as a deliberate interface change. The same applies to the solver metadata fields written into branch specs and run records.

\subsection{Persisted Raw-Data Contract}
The project now stores branch-local raw MAPF data under \texttt{dev/raw\_mapf\_data/<map\_type>/}. That store is split into a manifest, metadata files, and per-condition subfolders rather than one monolithic pickle. It is reused manually through \texttt{recompute\_MAPF} rather than validated automatically against code or configuration. If compute-stage data shape changes, update both the writer/reader logic and the documentation, and assume older saved layouts may need migration or replacement.
